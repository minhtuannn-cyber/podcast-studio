...
# --- Custom Voices API ---
CUSTOM_VOICES_FILE = "custom_voices.json"
REFS_DIR = "sovits_refs"
...
async def get_custom_voices():
    return {"voices": load_custom_voices()}
...
async def add_custom_voice(
    ...
    voice_id = f"custom_sovits_{int(time.time())}"
    ...
    file_path = os.path.join(REFS_DIR, safe_filename)
    ...
    new_voice = {
        ...
        "style": "Giọng Clone",
        "gender": "unknown",
        "tags": ["AI Clone", "Custom"],
        "voice": voice_id,
        "rate": "+0%",
        "pitch": "+0Hz",
        "description": description,
        "ref_audio_path": f"backend/{file_path}", # path relative to GPT-SoVITS root later if needed, but we pass absolute from backend. wait, let's just store relative to backend.
        "ref_text": ref_text,
        "ref_lang": language,
        "text_lang": language
    }
    # Fix the ref_audio_path to be absolute path so httpx can just pass it directly if needed, or pass relative.
    # Actually, GPT-SoVITS server accepts absolute path to audio file on the same machine.
    new_voice["ref_audio_path"] = os.path.abspath(file_path)
    
    voices.append(new_voice)
    save_custom_voices(voices)
    
    return {"message": "Voice added successfully", "voice": new_voice}
